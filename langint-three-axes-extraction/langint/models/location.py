...
class Embeddings(nn.Module):
    def __init__(self, num_placeholder_words: int, initializer_word: Optional[str], num_placeholder_groups: int = 2, shared_tokens=0, gt_init=0, blip_guidance=-1, fruit_blip_coeff=0, mat_blip_coeff=0, color_blip_coeff=0):
        ...
        assert num_placeholder_groups == 4, num_placeholder_groups
    ...
    def _process_clip_features(self, data, tokenizer, text_encoder):
        ...
        for i in range(len(data['prompt'])):
            ...
            ph_words_for_curr_prompt = [None, None, None, None]
            for word in words:
                if word in self.trainable_embeddings:
                    _, number = word.split("mytoken")
                    assert ph_words_for_curr_prompt[int(number) % 4] == None, (ph_words_for_curr_prompt[int(number) % 4], prompt)
                    ph_words_for_curr_prompt[int(number) % 4] = word
            ...
            assert ph_words_for_curr_prompt[3] != None, prompt 
        ...
        for ph_words, emb in zip(ph_words_by_prompt, x):
            assert len(emb) == len(ph_words) == 4, (len(emb), len(ph_words), ph_words)
            ...
            assert len(self.trainable_embeddings[ph_location]) == len(emb_location), (len(self.trainable_embeddings[ph_location]), len(emb_location))
            self.trainable_embeddings[ph_location] = nn.Parameter(emb_location.to(torch.float32).clone())
