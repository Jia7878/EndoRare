    ...
    def _visualize_core(self, data, prefix=None):
        ...
        with torch.no_grad():
            if 'val' in prefix:
                ...
                images = [data['image']*0.5 + 0.5]

                out = model({'prompt': data['prompt']}, return_all=True)
                fake_data = {'image': zero_image.repeat(len(data['prompt']), 1, 1, 1), 'prompt': data['prompt']}
                img = self.loss_modules['textual_inversion'].visualize({'embeddings': out['embeddings']}, fake_data,args.opt_embs_file_path)['image']
                ...
                rand_gt_mats = ['adenomatous polyp with low-grade epithelial dysplasia', 'tubular adenoma with high-grade intraepithelial neoplasia', 'adenomatous polyp with low-grade epithelial dysplasia', 'tubular adenoma with high-grade intraepithelial neoplasia', 'adenomatous polyp with low-grade epithelial dysplasia', 'tubular adenoma with high-grade intraepithelial neoplasia']
                ...
                perms = [
                    [rand_gt_fruits, rand_gt_colors, new_gt_mats, rand_ph_fruits, rand_ph_colors, new_ph_mats],  # 交换了颜色和材质的位置
                    [rand_gt_fruits, new_gt_colors, rand_gt_mats, rand_ph_fruits, new_ph_colors, rand_ph_mats],  # 交换了颜色和材质的位置
                    [new_gt_fruits, rand_gt_colors, rand_gt_mats, new_ph_fruits, rand_ph_colors, rand_ph_mats],  # 交换了颜色和材质的位置
                ]
                ...
                for perm in perms:
                    ...
                    ph_comp_prompts = []
                    comp_imgs = []
                    for gt_fruit, gt_color, gt_mat, ph_fruit, ph_color, ph_mat in zip(*perm):
                        gt_comp_prompts.append(TEMPLATE.format(gt_fruit, gt_color, gt_mat))
                        ph_comp_prompts.append(TEMPLATE.format(ph_fruit, ph_color, ph_mat))
    ...
    def validate(self, dataloader):
        for ind, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                """
                'prompt' =['a mytoken9 polyp with mytoken1 and pathology is mytoken2']
                'gt_prompt' =['a IIa type polyp with red color and pathology is tubular adenoma with low-grade epithelial dysplasia']
                'gt_fruit' =['IIa type']
