...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    # 如果没有传入 text 参数，使用 inputs_embeds 或 input_ids 作为输入
        if text is not None:
            # 如果传入了文本，使用 tokenizer 将其转化为 input_ids
            batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                            return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
            tokens = batch_encoding["input_ids"].to(self.device)
        elif input_ids is not None:
            # 如果传入了 input_ids，直接使用它
            tokens = input_ids
        ...
        # 如果传入的是输入嵌入（inputs_embeds），则直接将其传递给 transformer
        if inputs_embeds is not None:
            # 调用 CLIP 模型的 transformer，直接使用嵌入
            outputs = self.transformer(inputs_embeds=inputs_embeds).last_hidden_state
        else:
            # 否则使用 input_ids 进行编码
            outputs = self.transformer(input_ids=tokens.to(self.device))

        # 检查 outputs 是否是 Tensor 类型
