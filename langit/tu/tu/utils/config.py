...
def get_obj_from_str(string, reload=False):
    """根据传入的字符串，动态地从模块中导入类或函数。
    """

    module, cls = string.rsplit(".", 1)  # 一个字符串，表示模块和类（或函数）的全名，格式为 "module_name.class_name"
    if reload:
        module_imp = importlib.import_module(module)
        importlib.reload(module_imp)
    return getattr(importlib.import_module(module, package=None), cls)  # 使用 getattr 获取模块中的类或函数（由 cls 指定）
...
def build_from_config(config, **kwargs):
    ...
    if isinstance(config, str):
        return get_obj_from_str(config)  # 如果配置是字符串，直接获取对象

    # 如果配置是一个字典
    obj = get_obj_from_str(config['__target__'])  # 从配置中获取目标对象（类或函数）
    return obj(**config['kwargs'], **kwargs)  # 用配置中的参数和额外参数实例化对象
...
def overwrite_cfg_thorough_recursive(cfg: Dict, key: str, value: Any):
    if key in cfg: 
        cfg[key] = value
        logger.info(f'overwrite key {key}: {cfg.get(key)} -> {value}')
    for k in cfg:
        if isinstance(cfg[k], dict):
            overwrite_cfg_thorough_recursive(cfg[k], key, value)
    return cfg
