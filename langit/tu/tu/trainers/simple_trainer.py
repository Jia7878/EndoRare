...
class BaseTrainer:
    def __init__(self, modules: Dict[str, nn.Module],
        ...
        #* modules[key] = embeddings
        #  # 模型模块{"mokuai1": encoder1, "mokuai2": encoder2, "mokuai3": encoder3, "mukuai4": InvertDeepFloyd}
        if len(modules) == 1:
    ...
    def train_step(self, data, module_key=None, print_every=100, writer=None):
        ...
        module = self.modules[module_key]  #* embeddings
    ...
    def visualize(self, data):     #* 1.调用这个可视化
    ...
    def _visualize_core(self, data, prefix=None):
        ...
        model.eval()
        out = model.visualize(data)  #* 2.调用这个可视化
...
def train_loops(print_every, visualize_every, checkpoint_every, eval_every,
    ...
    trainer.visualize(process_batch(next(iter(train_loader))))  #!visualize
    # exit()
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            if eval_every > 0 and (trainer.it % eval_every == 0  or trainer.it == 100):
                ...
                trainer.validate(val_loader)  #! validate
...
#                  loss_modules: Dict[str, Dict],
#                  loss_weights: Dict[str, float],
#                  writer, title=None, it=-1, checkpoint_dir=None, checkpoint_dir2=None, images_dir=None, colors=None, model_mode=None, change=None, token=None):
#         self.modules = modules
#         if len(modules) == 1:
#             self.module_key, = modules.keys()
#         else:
...
#         for k, v in state_dict['modules'].items():
#             self.modules[k].load_state_dict(v, strict=False)
#         for k, v in state_dict['optimizers'].items():
#             self.optimizers[k].load_state_dict(v)
#             for param_group in self.optimizers[k].param_groups:
#                 param_group['lr'] = old_lr[k]
...
#     def load_two_checkpoints(self, path, path2):
#         # we do not need to go through the hassle of load_state_dict and replacing an initialized model's weights.
#         # just get the raw tensors and call visualize.
#         # then in visualize function, hardcode which tensors we want.
#         if path is not None and path2 is not None:
#             state_dict = torch.load(path)
#             state_dict2 = torch.load(path2)
            
#             assert len(state_dict['modules'].items()) == 1, state_dict['modules'].items()
#             assert len(state_dict2['modules'].items()) == 1, state_dict2['modules'].items()

#             v = list(state_dict['modules'].values())[0]
#             v2 = list(state_dict2['modules'].values())[0]
#         else: 
#             v = 'None'
#             v2 = 'None'


#         self.mix_datasets(v, v2)
...
# def load_mixed_checkpoint(trainer, cfg, path, path2):
#     epoch = trainer.load_two_checkpoints(path, path2)['epoch']
#     return epoch
...
#                     for k, v in loss.items():
#                         if v is None:
#                             continue  # 如果 v 是 None，跳过这项
#                         if isinstance(v, torch.Tensor) and v.dtype == torch.float32:
#                             v = v.float()  # 确保 v 是 float 类型的 Tensor
#                         # 如果 v 仍然是 None，就跳过这项
#                         if v is None:
#                             continue
#                         trainer.writer.add_scalar(k, v, trainer.it)
...
#                     info_txt = '[Epoch %02d] it=%03d, time=%.3f' % (epoch, trainer.it, time.time() - t0b)
#                     for (k, v) in loss.items():
#                         # 如果 v 是 None，则跳过该项或用默认值 -1 替代
#                         if v is None:
#                             info_txt += ', %s: -1.0000' % k  # 你可以选择用 -1 或其他合适的默认值
#                         else:
#                             info_txt += ', %s: %.4f' % (k, v)
#                     logger.info(info_txt)
#                 t0b = time.time()
