...
def get_parser():
    ...
    parser.add_argument('-o', '--overwrite', default=True,action='store_true', help='overwrite output dir')
    ...
    # parser.add_argument('-embs','--opt_embs_file_path', type=str, required=True, help="Path to the optimization embeddings file.")
...
def get_log_dir(cfg, args):
    ...
    if args.tag is not None:
        tag += f'_{args.tag}'
    if os.environ.get('SLURM_JOB_NAME') == 'bash' or os.environ.get('SLURM_JOB_NAME') is None:
        tag += '_local'
...
def get_cfg(args, overwrite_fn_before_resolve=None):
    ...
    cfg = load_cfg_from_path(cfg_path)  # 从 yaml 或者 json 文件中加载参数字典
    # overwrite_cfg(cfg, "dataset", args.dataset)
    overwrite_cfg(cfg, "title", args.tag)
