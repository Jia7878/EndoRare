...
def main():
    ...
    train_loader, val_loader = build_dataloaders(cfg, seed=args.seed, use_ddp=dist.is_initialized()) ##langint.datasets.glide.SyntheticBiLevel("data_root": "gt1-gt2-gt3")
    # overwrite_cfg_thorough_recursive(cfg, 'num_placeholder_words', train_loader.dataset.num_placeholder_words)
    # print('config', cfg)
    modules, optimizers, schedulers = build_training_modules(cfg, use_ddp=dist.is_initialized())  #* modules[key] = embeddings
    ...
    trainer = build_from_config(cfg['trainer'], modules=modules, writer=writer, optimizers=optimizers, schedulers=schedulers)  # 返回一个 Trainer 类。

    epoch = load_checkpoint(trainer, cfg, cfg['training']['checkpoint_dir'])

    build_from_config(cfg['training']['train_loops_fn'], cfg=cfg,
                      trainer=trainer, train_loader=train_loader, val_loader=val_loader, epoch=epoch)  # 调用 cfg['training']['train_loops_fn'] 里指定的训练函数 tu.trainers.simple_trainer.train_loops 进行训练
